...
``` 
The **"!access(fn, W_OK)"** is there for the program to see if the user has the permissions to change the file and the **"fp = fopen(fn, "a+")"** opens the file.

But how is this program vulnerable? We run this 3 lines:
...
This 3 lines compile the program and make it run with **SUID**, this means that when any user runs this program, this process is executed as **root**! So by leveriging this process using the race condition we can potentially write to any file, including to **/etc/passwd** and introducing our own root user.
...
We can see that the user **root** has ID 0 and password of **x**. This might seem a bit strange but x menas that the password's hash is the etc/shadow file.


An interesting feature is that Ubuntu has a certain password that allows for an password-less account. This password being: **U6aMy0wojraho**. As we see bellow we can create an account wit it and we can log in without giving the password.
...
With the access control files explored we will now finally attack the system. But let us start with an easy example first. The race condition vulnerability only works if we can execute it in between the **access** and **open** lines in the vulp script. So to facilitate this exploit we will add a **sleep(10)** in between those lines to allow us to better explot it, so we alter the vulp.c as such:
...
2. Exectue the program and writing a new user:
```sh
$ ./vulp
...
```
An we've done it! As we illustrate the exploit with the following images:
...
Timing is key for this exploit, we only have root priveleges for a very short window of time so the way to do it is to execute or exploit in that time. This is very hard for a human but, luckily, we can craft a script that attempts it as many times it takes to get it to work.
For this exploit we made two bash scripts that run at the same time, one that deletes the **/tmp/XYZ** file, creates it again and links it to **/etc/passwd** until it sees that the **/etc/passwd** has changed. The second runs the vulp script with the same input on a file that being: exp:U6aMy0wojraho:0:0:exp:/root:/bin/bash. We have the two scripts bellow:
...
It took us arround 2 minutes for it to work, but has we found out it does not always work. That is because, with the script above, we created our own race condition! When we delete **/tmp/XYZ**, create it again and link it the vulp script might beat us to the file creation and create **/tmp/XYZ** himself. That might not seem like a problem but now the file is created and owned by root and that means that we cannot delete it anymore and change its symbolic link and so the exploit fails.
There is a solution, we can use a C library to swap symbolic links without removing the **/tmp/XYZ**.
